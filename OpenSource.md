# Open Source Licenses

This project uses or refers to the following open-source software, libraries, and designs. We comply with the license terms of each software.

## Design Reference

### [al-folio](https://github.com/alshedivat/al-folio)
- **License**: MIT License
- **Usage**: Referenced for the overall website design and layout structure.

## Core Framework & Build Tools

### [Svelte](https://svelte.dev/) / [SvelteKit](https://kit.svelte.dev/)
- **License**: MIT License
- **Usage**: Used for frontend framework and application architecture development.

### [Vite](https://vitejs.dev/)
- **License**: MIT License
- **Usage**: Used as the frontend build tool and development server.

### [TypeScript](https://www.typescriptlang.org/)
- **License**: Apache License 2.0
- **Usage**: Used for static typing and code quality management.

## UI & Styling

### [Tailwind CSS](https://tailwindcss.com/)
- **License**: MIT License
- **Usage**: Used as a utility-first CSS framework for styling.

### [Lucide Icons](https://lucide.dev/) (Associated with Bits UI, etc.)
- **License**: ISC License
- **Usage**: Used for user interface icons.

### [Font Awesome Free](https://fontawesome.com/)
- **License**: 
  - Code: MIT License
  - Fonts: SIL OFL 1.1
  - Icons: CC BY 4.0
- **Usage**: Used for additional icons and web fonts.

### [Bits UI](https://www.bits-ui.com/)
- **License**: MIT License
- **Usage**: Used as an accessible headless UI component library.

## Utilities

### [Internationalized Date](https://react-spectrum.adobe.com/internationalized/date/)
- **License**: Apache License 2.0
- **Usage**: Used for handling internationalized dates and times.

### [JSZip](https://stuk.github.io/jszip/)
- **License**: MIT License or GPLv3
- **Usage**: Used for client-side file compression and download functionality.

### [Sharp](https://sharp.pixelplumbing.com/)
- **License**: Apache License 2.0
- **Usage**: Used for high-performance image processing.

### [LiquidJS](https://liquidjs.com/)
- **License**: MIT License
- **Usage**: Used for implementation of the Liquid template engine.

### [Octokit](https://github.com/octokit/rest.js)
- **License**: MIT License
- **Usage**: Used as a client for GitHub API integration.
